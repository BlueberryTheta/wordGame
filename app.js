...
function openGameOver(title, message) {
  ...
  // Build score if available (derive from DOM if not passed)
  try {
    ...
    if (scoreWrap) {
      ...
      if (gUsed != null && qUsed != null && t?.textContent?.toLowerCase().includes('got')) {
        ...
        if (shareBtn) {
          shareBtn.onclick = () => shareScore({ gUsed, qUsed });
        }
      } else {
...
async function shareScore({ gUsed, qUsed }) {
  try {
    ...
    if (navigator.clipboard && window.ClipboardItem) {
      ...
      if (btn) { const prev = btn.textContent; btn.textContent = 'Copied image!'; setTimeout(()=>btn.textContent=prev, 1500); }
      return;
    }
    // Download fallback
